...
import cv2
import numpy as np
from PySide.QtGui import QApplication, QWidget, QGraphicsScene, QGraphicsView, QVBoxLayout, QHBoxLayout, QToolBar
from PySide.QtGui import QImage, QPixmap, QGraphicsTextItem, QGraphicsPixmapItem, QColor
...
class ImageViewer(QWidget):
    ...
    def set_image(self, img: np.ndarray):
        ...
        # update image in the scene
        h, w, _ = im.shape
        bytes_per_line = w*3
        im = QImage(im.data, w, h, bytes_per_line, QImage.Format_RGB888).rgbSwapped()   # bgr to rgb
        w, h = self.view.width(), self.view.height()
        pix_map = QPixmap.fromImage(im).scaled(w, h, Qt.KeepAspectRatio)
